...
def get_average_scores(directors):
    ...
    #         pass
    #     else:
    #         avg_score = _calc_mean(movies)

    # return {(director, avg_score): movies}
...
def print_results(directors):
    ...
    for counter, (director_rating, movies) in enumerate(
        sorted(directors.items(), key=lambda x: float(x[0][1]), reverse=True)[
            :NUM_TOP_DIRECTORS
        ],
        1,
    ):
        ...
        for m in sorted(movies, key=lambda m: m.score, reverse=True):
            print(fmt_movie_entry.format(year=m.year, title=m.title[:50], score=m.score))
